<template>
  <div>
    <section
      class="bg-gradient-to-br from-primary-600 via-primary-700 to-indigo-900 py-20 text-white"
    >
      <div class="container-page text-center">
        <h2 class="mb-4 text-4xl font-bold tracking-tight md:text-5xl">Welcome to APAN Store</h2>
        <p class="mx-auto mb-8 max-w-2xl text-lg text-indigo-200">
          Discover our curated collection of products. Shop with confidence and enjoy a seamless
          experience.
        </p>
        <NuxtLink
          to="/products"
          class="inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-semibold text-primary-700 shadow-soft transition-colors hover:bg-indigo-50"
        >
          Browse Products
    ...
    <section class="container-page py-16">
      ...
      <div v-else-if="products.length === 0" class="rounded-lg bg-gray-50 p-16 text-center">
        <p class="text-text-muted">
          Configure your Shopify credentials in the
          <code class="rounded bg-gray-200 px-2 py-0.5 text-sm font-mono">.env</code> file to
          display products.
        </p>
      </div>
...
const {
  ...
  error,
  status,
} = useAsyncData('home-products', () => getProducts(4), {
