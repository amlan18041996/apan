  ...
  <div v-else-if="product" class="container-page py-8">
    ...
    <div class="grid gap-8 md:grid-cols-2">
      ...
      <div>
        ...
        <p class="mt-4 text-3xl font-bold text-primary-600">
          ${{ formatPrice(product.priceRangeV2.minVariantPrice.amount) }}
        </p>
        <p v-if="product.status === 'ACTIVE'" class="mt-1 text-sm text-green-600">
          In Stock ({{ product.totalInventory }} available)
        </p>
          ...
          <div v-for="option in product.options" :key="option.id">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :key="val"
                class="rounded-lg border px-4 py-2 text-sm transition-colors hover:border-primary-500 hover:text-primary-600"
                :class="
        ...
        <div class="mt-6 flex items-center gap-4">
          <div class="flex items-center rounded-lg border border-border">
            <button
              class="flex h-10 w-10 items-center justify-center text-text-muted transition-colors hover:bg-gray-100"
              @click="quantity = Math.max(1, quantity - 1)"
            >
              -
            </button>
            <span class="flex h-10 w-12 items-center justify-center text-sm text-text">{{
              quantity
            }}</span>
            <button
              class="flex h-10 w-10 items-center justify-center text-text-muted transition-colors hover:bg-gray-100"
              @click="quantity = Math.min(product.totalInventory || 99, quantity + 1)"
            >
              +
            </button>
          </div>
