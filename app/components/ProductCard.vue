<template>
  <div class="card group relative overflow-hidden transition-shadow hover:shadow-soft-lg">
    <div class="absolute right-3 top-3 z-10 flex translate-x-2 flex-col gap-2 opacity-0 transition-all duration-200 group-hover:translate-x-0 group-hover:opacity-100">
      <button class="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-md transition-colors hover:bg-gray-100" @click.stop aria-label="Add to wishlist">
        <svg class="h-5 w-5 text-gray-600 transition-colors hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
      </button>
      <button class="flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-md transition-colors hover:bg-gray-100" @click.stop aria-label="Compare">
    ...
    <div class="p-4">
      <NuxtLink :to="`/products/${product.handle}`">
        <h3 class="mb-2 text-sm font-medium text-text line-clamp-2 hover:text-primary-600 transition-colors">{{ product.title }}</h3>
      </NuxtLink>

      <div class="flex items-center justify-between">
        <span v-if="isAvailable" class="inline-flex items-center gap-1 text-xs font-medium text-green-600">
        ...
        <div class="flex items-center gap-1">
          <svg v-for="i in 5" :key="i" class="h-3.5 w-3.5" :class="i <= Math.round(rating) ? 'text-yellow-400' : 'text-gray-300'" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
          <span class="ml-0.5 text-xs text-text-muted">({{ rating }})</span>
        </div>
      </div>
    </div>
  </div>
